...
- On this machine, mobile runtime work should currently use the short junction path `C:\sbm\mobile`; the original repo path is long enough to trigger Windows-specific toolchain problems.
...
The latest short-path Expo startup attempt changed the failure mode materially: `cd /c/sbm/mobile && npx expo start --android --port 8081` did not reproduce the malformed Metro watcher crash, and instead reported that no development build for `io.pcschmidt.signalbrief` is installed.

The latest short-path Android dev build also progressed beyond the earlier fast-fail path-length state: `cd /c/sbm/mobile && npm run android:dev-client -- --port 8082` reached deep Gradle configuration and dependency resolution before work stopped.
...
Use `C:\sbm\mobile` as the default path for mobile commands on this machine. If the Android dev build still fails there after a full run, then revisit Node LTS changes or Windows long-path settings.
